...
def encode_image(image_path, vae, device, target_size=(256, 256)):
    """
    Load an image from the given path, resize it to target_size (height, width),
    and encode it into a latent representation via the VAE.
    """
    ...
    # Convert to RGB to match expected channel count
    image = image.convert("RGB")

    preprocess = transforms.Compose([
        transforms.Resize(target_size),  # Resize to target_size (e.g. (320, 480))
        transforms.ToTensor(),
    ...
    try:
        with torch.no_grad():
            # Use VAE's actual scaling factor instead of hardcoded value
            # latents = vae.encode(image_tensor).latent_dist.sample() * vae.config.scaling_factor
            latents = vae.encode(image_tensor).latent_dist.mean * vae.config.scaling_factor

        logger.info(f"Latents encoded successfully with shape: {latents.shape}")
        logger.info(f"Using VAE scaling factor: {vae.config.scaling_factor}")
        return latents
    except Exception as e:
...
class End2End(object):
    def __init__(self, args, models_root_path):
        ...
        # Disable CLIP model logging checkpoint info
        tf_logging.set_verbosity_error()

        # ========================================================================
        logger.info(f"Loading CLIP Text Encoder...")
        text_encoder_path = self.root / "clip_text_encoder"
        self.clip_text_encoder = CLIPTextModel.from_pretrained(str(text_encoder_path)).to(self.device)
    ...
    def load_torch_weights(self):
        ...
        # Use strict=False for compatibility; missing keys will be initialized automatically
        missing_keys, unexpected_keys = self.model.load_state_dict(state_dict, strict=False)

        if missing_keys:
            logger.info(f"Initializing missing keys: {len(missing_keys)} keys")
            # input_img_embedder and new skip layers are initialized automatically
            for key in missing_keys:
                if 'input_img_embedder' in key:
                    logger.info(f"  - New input image embedder: {key}")
                elif 'skip_norm_with_input' in key or 'skip_linear_with_input' in key:
                    logger.info(f"  - New skip connection layer: {key}")
        
        if unexpected_keys:
            logger.info(f"Found unexpected keys (will be ignored): {len(unexpected_keys)} keys")
    ...
    def predict(self,
                ...
                strength=0.8,    # Strength for image-to-image transformation (0.0 = no change, 1.0 = completely new)
                seed=None,
        ...
        # ========================================================================
        # Handle image_latents if image_path is provided
        if image_path is not None:
            logger.info(f"Encoding image from path: {image_path}")
            # args.image_size is passed as a list [height, width], convert to tuple
        ...
        if self.infer_mode in ['fa', 'torch']:
            # Force height and width to align to 16
        ...
        # ========================================================================
        # Inner arguments: image_meta_size
        # ========================================================================
        ...
        # Now include image_latents and strength in the pipeline call
        samples = self.pipeline(
            ...
            strength=strength,  # Add strength parameter for image-to-image
            ...
            input_image=image_latents,  # Pass clean input image for concatenation
        )[0]
