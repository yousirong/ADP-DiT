...
class GaussianDiffusion:
    ...
    def dpmpp_2m_sde_exponential_sample(
        ...
        """
        DPM++ 2M SDE Exponential sampler function.

        :param model: Diffusion model.
        :param x: Current state sample.
        :param t: Current time step.
        :param clip_denoised: If True, clip x_start to [-1, 1].
        :param denoised_fn: Additional function applied after predicting x_start.
        :param model_kwargs: Extra arguments passed to the model.
        :return: Next sample and predicted x_start.
        """
        ...
        # Compute model output and predict initial state
        ...
        # Exponential scheduling computation
    ...
    def dpmpp_3m_sde_karras_sample(
        ...
        """
        DPM++ 3M SDE Karras sampler function.
        ...
        # Compute model output and predict x_start
        out = self.p_mean_variance(
            model,
            x,
            t,
    ...
    def dpmpp_3m_sde_exp_sample(
        ...
        """
        DPM++ 3M SDE Exponential sampler function.
        ...
        # Compute time step difference (exponential scheduling)
    ...
    def training_losses(self, model, x_start, model_kwargs=None, controlnet=None, noise=None, cls_labels=None):
        """
        Compute training losses for text-guided image-to-image.

        For img2img training:
        - x_start: target latents (edited_image latents) - what we want to generate
        - model_kwargs['input_latents']: condition latents (input_image latents) - the condition

        The model learns to predict the target image given the input image and text prompt.

        :param model: the model to evaluate loss on.
        :param x_start: the [N x C x ...] tensor of target latents (edited image).
        :param model_kwargs: dict containing 'input_latents' for conditioning.
        :param noise: if specified, the specific Gaussian noise to try to remove.
        :param cls_labels: classification labels (0: CN, 1: MCI, 2: AD) for classification loss.
        :return: a dict with the key "loss" containing a tensor of shape [N].
        """
        if model_kwargs is None:
            model_kwargs = {}

        # Extract input_latents (condition) from model_kwargs
        input_latents = model_kwargs.get('input_latents', None)

        # Time steps
        t = th.randint(0, self.num_timesteps, (x_start.shape[0],), device=x_start.device)

        # Noise for the target latents
        if noise is None:
            noise = th.randn_like(x_start)
        if self.noise_offset > 0:
            noise = noise + self.noise_offset * th.randn(*x_start.shape[:2], 1, 1, device=x_start.device)

        # Add noise to target latents (standard diffusion forward process)
        x_t = self.q_sample(x_start, t, noise=noise)

        # For img2img: pass input_latents as input_image to the model
        # The model uses skip connection mechanism to condition on input image
        if input_latents is not None:
            model_kwargs['input_image'] = input_latents
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            # Pass the noisy target to the model (input_image is already in model_kwargs)
            out_dict = model(x_t, t, **model_kwargs)
            model_output = out_dict['x']

            # Extract classification logits if available
            cls_logits = out_dict.get('cls_logits', None)
            extra = {k: v for k, v in out_dict.items() if k not in ['x', 'cls_logits']}
            ...
            # Target for the loss (what the model should predict)
            ...
            # Add classification loss if labels and logits are provided
            if cls_labels is not None and cls_logits is not None:
                # Class weights to handle imbalance (based on dataset distribution)
                # CN: 22.7%, MCI: 58.3%, AD: 19.0%
                # Inverse frequency weights normalized: CN=2.57, MCI=1.00, AD=3.07
                class_weights = th.tensor([2.57, 1.00, 3.07], device=cls_labels.device, dtype=cls_logits.dtype)

                # Classification loss (CrossEntropy with class weights)
                ce_loss = th.nn.functional.cross_entropy(cls_logits, cls_labels, weight=class_weights, reduction='none')
                terms["ce"] = ce_loss

                # Store individual losses for logging
                terms["mse_only"] = terms["mse"]
                terms["ce_only"] = ce_loss

                # Store classification logits and labels for accuracy calculation
                terms["cls_logits"] = cls_logits
                terms["cls_labels"] = cls_labels

                # Combined loss will be computed by CombinedLoss in training script
                # For now, just return the individual components
                if "vb" in terms:
                    terms["loss"] = terms["mse"] + terms["vb"]  # Keep as diffusion loss for now
                    terms["raw_loss"] = raw_mse + terms["vb"].detach()
                else:
                    terms["loss"] = terms["mse"]  # Keep as diffusion loss for now
