...
def _module_device(module: Optional[torch.nn.Module]) -> Optional[torch.device]:
...
def _resolve_pipeline_device(pipeline) -> torch.device:
    ...
    for name in ("unet", "vae", "text_encoder", "text_encoder_2"):
        device = _module_device(getattr(pipeline, name, None))
        if device is not None:
            return device

    embedder = getattr(pipeline, "embedder_t5", None)
    if embedder is not None:
        device = _module_device(getattr(embedder, "model", None))
        if device is not None:
            return device
...
class StableDiffusionPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    Args:
        ...
        text_encoder (Optional[`~transformers.CLIPTextModel`]):
            Frozen CLIP text-encoder (CLIP-G bigG-14).
        tokenizer (Optional[`~transformers.CLIPTokenizer`]):
    ...
    def encode_prompt(
        ...
        if text_encoder is not None:
            prompt_embeds_dtype = getattr(text_encoder, "dtype", None)
            if prompt_embeds_dtype is None:
                try:
                    prompt_embeds_dtype = next(text_encoder.parameters()).dtype
                except StopIteration:
                    prompt_embeds_dtype = None
        elif self.unet is not None:
            prompt_embeds_dtype = getattr(self.unet, "dtype", None)
            if prompt_embeds_dtype is None:
                try:
                    prompt_embeds_dtype = next(self.unet.parameters()).dtype
                except StopIteration:
                    prompt_embeds_dtype = None
        else:
            prompt_embeds_dtype = None
    ...
    def prepare_latents(self, batch_size, num_channels_latents, height, width, dtype, device, generator, latents=None, is_image_latents=False):
        ...
        if latents is None:
            ...
            # For image-to-image, don't add noise scaling to preserve input image information
            if not is_image_latents:
                latents = latents * self.scheduler.init_noise_sigma
    ...
    def __call__(
            ...
            strength: Optional[float] = 0.8,  # Strength for image-to-image (0.0 = no change, 1.0 = completely new)
            prompt_embeds: Optional[torch.FloatTensor] = None,
            ...
            input_image: Optional[torch.FloatTensor] = None,  # Clean input image for concatenation
    ):
        r"""
        ...
        device = _resolve_pipeline_device(self)
        ...
        # For image-to-image, adjust timesteps based on strength (classic img2img approach)
        # Timesteps are ordered from high noise to low noise: [999, 989, ..., 10]
        # strength controls how much noise to add:
        #   strength=1.0: start from t=999 (full noise), use all timesteps → t=10
        #   strength=0.6: start from t~600 (medium noise), denoise → t=10
        #   strength=0.1: start from t~100 (low noise), denoise → t=10
        is_image_to_image = image_latents is not None
        if is_image_to_image:
            # Calculate how many steps to use based on strength
            actual_steps = max(int(num_inference_steps * strength), 1)
            # Skip the first (num_inference_steps - actual_steps) timesteps
            # This means we start from a lower noise level and always denoise to t≈0
            t_start = max(num_inference_steps - actual_steps, 0)
        ...
        # Prepare latents first
        # For image-to-image, preserve input latents; for text-to-image, generate new ones
        ...
        latents = self.prepare_latents(batch_size * num_images_per_prompt,
                                       ...
                                       generator,
                                       latents,
                                       is_image_latents=is_img2img,  # Preserve input latents for image-to-image
                                       )

        # For image-to-image, add noise according to the first timestep we'll process
        if is_image_to_image and strength > 0.01 and len(timesteps) > 0:
            # Generate noise
            noise = randn_tensor(latents.shape, generator=generator, device=device, dtype=latents.dtype)

            # Use the FIRST timestep from our schedule as the starting noise level
            # This is the highest noise level we'll denoise from
            init_timestep = timesteps[0:1].repeat(batch_size * num_images_per_prompt)

            # Add noise using scheduler's proper noise schedule
