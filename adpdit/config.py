...
def get_args(default_args=None):
    ...
    parser.add_argument("--text-states-dim", type=int, default=1280, help="Hidden size of CLIP-G text encoder.")
    parser.add_argument("--text-len", type=int, default=77, help="Token length of CLIP-G text encoder output.")
    ...
    parser.add_argument("--strength", type=float, default=0.8,
                        help="Strength for image-to-image transformation. 0.0 = no change, 1.0 = completely new image.")
    ...
    parser.add_argument('--use-auxiliary-encoder', action='store_true',
                        help="Enable auxiliary metadata encoder for clinical and cognitive data.")
    parser.add_argument('--no-auxiliary-encoder', dest='use_auxiliary_encoder', action='store_false')
    parser.set_defaults(use_auxiliary_encoder=False)

    # Classification Training Control
    parser.add_argument('--use-classification', action='store_true',
                        help="Enable classification training with joint loss and validation (default: False for pure generation)")
    parser.add_argument('--no-classification', dest='use_classification', action='store_false')
    parser.set_defaults(use_classification=False)

    # Classification head dropout
    parser.add_argument('--cls-dropout', type=float, default=0.3,
                        help="Dropout rate for classification MLP to prevent overfitting (default: 0.3)")

    # Label smoothing for classification
    parser.add_argument('--label-smoothing', type=float, default=0.1,
                        help="Label smoothing factor for CrossEntropy loss (default: 0.1)")

    # Mediffusion-style loss weighting
    parser.add_argument('--cls-loss-weight', type=float, default=0.001,
                        help="Weight (lambda) for classification loss in joint training. "
                             "Mediffusion used: 0.00005 (2%% labels), 0.00015 (5%% labels), 0.001 (10%%+ labels). "
                             "Default: 0.001 for fully-supervised setting (default: 0.001)")
    parser.add_argument('--cls-warmup-steps', type=int, default=0,
                        help="Number of warmup steps before enabling classification loss. "
                             "Mediffusion used 5000 steps to let diffusion model learn features first. "
                             "Set to 0 to disable warmup (default: 0)")

    parser.add_argument("--cfg-scale", type=float, default=1.0, help="Guidance scale for classifier-free.")
    parser.add_argument("--negative", type=str, default="significant changes, large anatomical shift, brain shrinkage, distorted anatomy, unrelated artifacts", help="Negative prompt.")
    ...
    parser.add_argument("--valid-index-file", type=str, required=False,
                        help="Path to validation index file (JSON) for classification overfitting detection")
    parser.add_argument("--valid-from-train", action="store_true",
                        help="Create validation dataset from train data using 5 representative patients "
                             "(one per disease progression scenario: CNtoCN, CNtoMCI, MCItoMCI, MCItoAD, ADtoAD)")
    parser.add_argument("--train-csv-path", type=str, default="./dataset/AD_meta/csvfile/image_text.csv",
                        help="Path to train CSV file for --valid-from-train option")
    parser.add_argument("--val-every", type=int, default=100,
                        help="Run validation every N training steps (default: 100)")
    parser.add_argument("--val-patience", type=int, default=3,
                        help="Early stopping patience: freeze classifier if validation accuracy does not improve "
                             "for N consecutive validations (default: 3)")
    parser.add_argument("--val-start-step", type=int, default=0,
                        help="Start validation from this step onwards (default: 0). Useful to avoid OOM during early training "
                             "with large validation sets (e.g., val-start-step=10)")

    # Sample Generation During Training
    parser.add_argument("--sample-every", type=int, default=0,
                        help="Generate sample images every N training steps (default: 0, disabled). "
                             "Uses the currently training model to generate samples for visual inspection.")
    parser.add_argument("--num-samples", type=int, default=4,
                        help="Number of sample images to generate per --sample-every interval (default: 4)")
    parser.add_argument("--sample-infer-steps", type=int, default=20,
                        help="Number of inference steps for sample generation (default: 20)")
    parser.add_argument("--sample-cfg-scale", type=float, default=6.0,
                        help="CFG scale for sample generation (default: 6.0)")
    parser.add_argument("--sample-strength", type=float, default=0.1,
                        help="Strength for img2img sample generation (default: 0.1)")
    parser.add_argument("--val-batch-size", type=int, default=None,
                        help="Per-GPU batch size for validation. If None, defaults to 50%% of training batch size. "
                             "Smaller batch size prevents OOM during validation with classification.")
    parser.add_argument("--val-grad-accu-steps", type=int, default=1,
                        help="Gradient accumulation steps for validation (default: 1, no accumulation during validation)")
