...
def add_tuning_arguments(parser):
    ...
    # CosineAnnealingRestarts-specific settings (total_steps/num_cycles/warmup_ratio)
    group.add_argument(
        '--num-cycles', type=int, default=5,
        help='Number of cosine restart cycles over the entire training (total steps).'
    )
    group.add_argument(
        '--warmup-ratio', type=float, default=0.1,
        help='Fraction of each cycle used for linear warmup (0 ≤ warmup_ratio < 1).'
    )
    group.add_argument('--t_mult', type=float, default=2.0,
                       help='Factor to increase cycle duration after each restart.')
    group.add_argument('--max_lr', type=float, default=0.0001,
                       help='Maximum learning rate.')
    group.add_argument('--min_lr', type=float, default=0.00001,
                       help='Minimum learning rate.')
    group.add_argument('--gamma', type=float, default=2.0,
                       help='Factor to decay max_lr after each restart.')
...
class CosineAnnealingWarmupRestarts(_LRScheduler):
    """
    Cosine Annealing with Warmup and Restarts based on total steps, number of cycles, and warmup ratio,
    with cycle length multiplier (t_mult) and max_lr decay factor (gamma).

    First cycle starts from max_lr and goes down to min_lr (no warmup).
    Subsequent cycles have warmup from min_lr to max_lr followed by cosine annealing.

    Args:
        optimizer (Optimizer): Wrapped optimizer.
        total_steps (int): Total number of training steps across all cycles.
        num_cycles (int): Number of cosine cycles (restarts).
        warmup_ratio (float): Fraction of each cycle used for linear warmup (0 <= warmup_ratio < 1).
        max_lr (float): Maximum learning rate.
        min_lr (float): Minimum learning rate.
        t_mult (float): Multiplier to increase cycle duration after each restart.
        gamma (float): Factor to decay max_lr after each restart.
        last_epoch (int): The index of the last batch (for resuming). Default: -1.
    """
    def __init__(
        self,
        optimizer,
        ...
        assert num_cycles >= 1, "num_cycles must be >= 1"
        assert t_mult >= 1.0, "t_mult must be >= 1"
        ...
        # Compute initial cycle length to satisfy geometric series sum = total_steps
        if t_mult == 1.0:
            self.first_cycle_steps = total_steps // num_cycles
        else:
            self.first_cycle_steps = int(
                total_steps * (t_mult - 1) / (t_mult ** num_cycles - 1)
            )
        self.cur_cycle_steps = self.first_cycle_steps
        self.warmup_steps = int(self.cur_cycle_steps * warmup_ratio)
        ...
        # Start from min_lr for all cycles (including first cycle)
        for pg in self.optimizer.param_groups:
            pg['lr'] = self.min_lr
        # also prime _last_lr so get_last_lr() is valid immediately
        self._last_lr = [self.min_lr] * len(self.optimizer.param_groups)

    def get_lr(self):
        # All cycles (including first): warmup followed by cosine annealing
        # This ensures num_cycles=1 behaves like Cosine Simple

        # Linear warmup phase (min_lr → max_lr)
        if self.step_in_cycle < self.warmup_steps and self.warmup_steps > 0:
        ...
        # Cosine annealing phase (max_lr → min_lr)
        cosine_step = self.step_in_cycle - self.warmup_steps
        total_cos = self.cur_cycle_steps - self.warmup_steps

        if total_cos <= 0:
            # Edge case: if warmup takes entire cycle, return max_lr
            return [self.max_lr for _ in self.optimizer.param_groups]

        return [
            self.min_lr + (self.max_lr - self.min_lr) *
            (1 + math.cos(math.pi * cosine_step / total_cos)) / 2
            for _ in self.optimizer.param_groups
        ]

    def step(self, epoch=None, last_batch_iteration=None, increment=None):
        # Advance batch counter
        self.step_in_cycle += 1
        # If end of cycle reached, restart
        if self.step_in_cycle >= self.cur_cycle_steps:
            self.cycle += 1
            self.step_in_cycle = 0
            # Update cycle length and warmup for new cycle
            self.cur_cycle_steps = int(self.first_cycle_steps * (self.t_mult ** self.cycle))
            self.warmup_steps = int(self.cur_cycle_steps * self.warmup_ratio)
            # Decay max_lr for new cycle
            self.max_lr = self.base_max_lr * (self.gamma ** self.cycle)
        # Update last_epoch for base class
