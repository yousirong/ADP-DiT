...
class DistributedRandomReplacementSampler(DistributedSampler):
    """
    A sampler for distributed training where each process (rank) selects data
    from the full dataset using random sampling with replacement.

    Each epoch, num_samples_per_replica samples (with possible duplicates) are selected.
    """
    def __init__(self, dataset, num_replicas=None, rank=None, num_samples_per_replica=None, seed=0, start_index=0):
        ...
        self.num_replicas = num_replicas
        self.rank = rank
        self.num_samples = num_samples_per_replica  # Number of samples each process draws per epoch
        self.seed = seed
        self._start_index = start_index
    ...
    def __iter__(self):
        ...
        # Randomly sample with replacement from dataset indices in range [start_index, len(dataset)-1]
        ...
        # Each process receives indices assigned in round-robin fashion.
